...
export default async function DashboardPage() {
  ...
  const documentAlertsTile = dueDocs && dueDocs.length > 0 ? (
    <Card className="bg-slate-900 border-slate-800">
      <CardHeader className="flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <DashboardGrid
        summary={summaryTile}
        driverCompliance={driverComplianceTile}
        maintenance={maintenanceTile}
        documentAlerts={documentAlertsTile}
        fuelSpend={<FuelSpendTile />}
        otherCharges={<OtherChargesTile />}
        operatingCosts={operatingCostsTile}
        complianceAlerts={complianceAlertsTile}
        fuelPrices={fuelPricesTile}
      />
    </div>
  )
}
