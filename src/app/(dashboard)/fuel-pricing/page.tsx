  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await sb
        ...
        .limit(61 * 5)
      setPrices((data as TGPPrice[]) || [])
      setLoading(false)
    }
    load()
  }, [])

  // Latest prices per terminal
  const latestByTerminal = TERMINALS.map(terminal => {
    const row = prices.filter(p => p.terminal === terminal)[0]
    return { terminal, row }
  })
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">Terminal gate prices (diesel, ¢/L inc. GST)</p>
      </div>
