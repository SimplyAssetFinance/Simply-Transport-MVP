...
// ---------------------------------------------------------------------------
// Queensland Fuel Prices Direct API proxy
// Env var: QLD_FUEL_TOKEN  (the SubscriberToken GUID)
// Docs: https://fppdirectapi-prod.fuelpricesqld.com.au
// ---------------------------------------------------------------------------

const QLD_BASE      = 'https://fppdirectapi-prod.fuelpricesqld.com.au'
const COUNTRY_ID    = 21
const GEO_LEVEL     = 3
const GEO_REGION_ID = 1   // Queensland
...
function authHeader(): string {
  return `FPDAPI SubscriberToken=${process.env.QLD_FUEL_TOKEN}`
}
...
export async function GET(request: NextRequest) {
  ...
  if (!process.env.QLD_FUEL_TOKEN) {
    return NextResponse.json({ stations: [], error: 'QLD_FUEL_TOKEN not configured' })
  }
  ...
  try {
    const [sitesRes, pricesRes, brandsRes] = await Promise.all([
      fetch(`${QLD_BASE}/Subscriber/GetFullSiteDetails?${params}`,          { headers, next: { revalidate: 3600 } }),
      fetch(`${QLD_BASE}/Price/GetSitesPrices?${params}`,                   { headers, next: { revalidate: 1800 } }),
      fetch(`${QLD_BASE}/Subscriber/GetCountryBrands?countryId=${COUNTRY_ID}`, { headers, next: { revalidate: 86400 } }),
    ])
    ...
    console.error('QLD Fuel API error:', err.message)
